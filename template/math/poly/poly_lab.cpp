...
void polysqrt(Z f[], int n)
{
    constexpr Z inv2 = Z(2).inv();
    static Z    g[MAXN], b[MAXN], c[MAXN];
    for (int i = 0; i < n; i++) g[i] = 0;
    g[0] = 1;  // 根据需要改成sqrt(f[0])
    for (int i = 2; i <= n; i <<= 1) {
        for (int j = 0; j < i; j++) b[j] = f[j];
        for (int j = i; j < 2 * i; j++) b[j] = 0;
        for (int j = 0; j < i / 2; j++) c[j] = g[j];
        for (int j = i / 2; j < i; j++) c[j] = 0;
        polyinv(c, i);
        for (int j = i; j < 2 * i; j++) c[j] = 0;
        convolution(b, c, 2 * i);
        for (int j = 0; j < i; j++) g[j] = inv2 * (g[j] + b[j]);
    }
