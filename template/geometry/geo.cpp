...
struct Point {
    ...
    bool operator==(const Point &a) const { return (abs(x - a.x) <= GEO_eps && abs(y - a.y) <= GEO_eps); }
    ...
    Point operator*(const GEO_T k) const { return {k * x, k * y}; }

    Point operator/(const GEO_T k) const { return {x / k, y / k}; }

    GEO_T operator*(const Point &a) const { return x * a.x + y * a.y; }  // 点积

    GEO_T operator^(const Point &a) const { return x * a.y - y * a.x; }  // 叉积，注意优先级
    ...
    GEO_T len2() const { return (*this) * (*this); }  // 向量长度的平方

    GEO_T dis2(const Point &a) const { return (a - (*this)).len2(); }  // 两点距离的平方
    ...
    // 必须用浮点数
    GEO_T len() const { return sqrtl(len2()); }  // 向量长度

    GEO_T dis(const Point &a) const { return sqrtl(dis2(a)); }  // 两点距离
...
struct Line {
    ...
    GEO_T dis(const Point &a) const { return abs(v ^ (a - p)) / v.len(); }  // 点到直线距离
...
struct Circle {
    ...
    bool operator==(const Circle &a) const { return c == a.c && abs(r - a.r) <= GEO_eps; }

    GEO_T circ() const { return 2 * GEO_PI * r; }  // 周长

    GEO_T area() const { return GEO_PI * r * r; }  // 面积
