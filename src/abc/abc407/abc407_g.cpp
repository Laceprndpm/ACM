...
void solve()
{
    int h, w;
    cin >> h >> w;
    auto                unzip = [&](int row, int col) { return row * (w + 1) + col; };
    MCFGraph            mcf(unzip(h, w) + 3 + 1);
    int                 s1 = unzip(h, w) + 1, s2 = s1 + 1, t = s2 + 1;
    int                 dx[] = {0, 0, 1, -1};
    int                 dy[] = {1, -1, 0, 0};
    i64                 ans  = 0;
    vector<vector<i64>> Ai(h + 1, vector<i64>(w + 1));
    for (int i = 1; i <= h; i++) {
        for (int j = 1; j <= w; j++) {
            cin >> Ai[i][j];
            ans += Ai[i][j];
        ...
        for (int j = 1; j <= w; j++) {
            if ((i + j) % 2 == 0) {
                for (int k = 0; k < 4; k++) {
                    ...
                    mcf.addEdge(unzip(i, j), unzip(nx, ny), 1, Ai[i][j] + Ai[nx][ny]);
                }
            }
        }
    }
    for (int i = 1; i <= h; i++) {
        for (int j = 1; j <= w; j++) {
            if ((i + j) % 2 == 0) {
                mcf.addEdge(s2, unzip(i, j), 1, 0);
            } else {
                mcf.addEdge(unzip(i, j), t, 1, 0);
            }
        }
    }
    i64 realans = ans;
    for (int flow = 1; flow <= h * w / 2; flow++) {
        mcf.addEdge(s1, s2, 1, 0);
        auto [fl, cost] = mcf.flow(s1, t);
        ans -= cost;
        realans = max(realans, ans);
    }
    function<i64(int, int)> add = [&](int a, int b) { return (a + b); };
    cout << realans << '\n';
}
