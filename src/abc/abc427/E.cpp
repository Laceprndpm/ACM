...
char graph[MAXN][MAXN][MAXN][MAXN][MAXN][MAXN];
...
void solve()
{
    ...
    auto trymove = [&](int x, int y) -> int {
        int ans = INF;
        if (x == 1) {
            ans = min(ans, h - mnXsharp + 1);
        }
        if (x == h) {
            ans = min(ans, mxXsharp);
        }
        if (y == 1) {
            ans = min(ans, w - mnYsharp + 1);
        }
        if (y == w) {
            ans = min(ans, mxYsharp);
        }
        return ans;
    };
        ...
        for (int mxX = mnX; mxX <= h + 1; mxX++) {
            for (int mnY = 0; mnY <= w + 1; mnY++) {
                for (int mxY = mnY; mxY <= w + 1; mxY++) {
                    ...
                    for (int x = 0; x <= h + 1; x++) {
                        for (int y = 0; y <= w + 1; y++) {
                            if ((mxX - tx) + 1 <= x && x <= h - (tx - mnX) && (mxY - ty) + 1 <= y && y <= w - (ty - mnY)
                                && board[x][y] == '#') {
    ...
    auto bfs  = [&]() -> int {
        ...
        que.pb({0, tx, ty, tx, tx, ty, ty});
        dp[tx][tx][ty][ty][tx][ty] = 1;
        while (!que.empty()) {
            auto [dis, x, y, mnX, mxX, mnY, mxY] = que.front();
            que.pop_front();
            if (ok[mnX][mxX][mnY][mxY]) {
                return min(ans, dis);
            }
            ans = min(ans, dis + trymove(x, y));
            for (int j = 0; j < 4; j++) {
                ...
                int nmnY = min(mnY, ny);
                int nmxY = max(mxY, ny);
                if (nx >= 0 && nx <= h + 1 && ny >= 0 && ny <= w + 1 && graph[mnX][mxX][mnY][mxY][nx][ny] == '.'
                    && !dp[nmnX][nmxX][nmnY][nmxY][nx][ny]) {
                    dp[nmnX][nmxX][nmnY][nmxY][nx][ny] = 1;
