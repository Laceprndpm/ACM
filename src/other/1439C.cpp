...
struct LazySegmentTree {
    ...
    int lower_bound_single_idx(int p, int l, int r, i64 k)  // 找到第一个小于等于k的元素的索引
    {
        ...
        int m = (l + r) / 2;
        push(p);
        int res = lower_bound_single_idx(p << 1, l, m, k);
        if (res == -1) {
            res = lower_bound_single_idx(p << 1 | 1, m + 1, r, k);
        }
        return res;
    }

    int lower_bound_single_idx(i64 k)  // 最高log(n)
                                       // 找到第一个小于等于k的元素的索引
    {
        if (info[1].range_min > k) return -1;
        return lower_bound_single_idx(1, 1, n, k);
    }

    int lower_bound_len_idx(i64 k)
    {
    ...
    int lower_bound_len_idx(int p, int l, int r, i64 k)  // 找到下一个节点的尾
    {
        ...
        i64 m = (l + r) / 2;
        push(p);
        i64 res = lower_bound_len_idx(p << 1, l, m, k);
        if (res < m) {
            return res;
        }
        res = lower_bound_len_idx(p << 1 | 1, m + 1, r, k - info[p << 1].range_sum);
        return res;
    }
};
    ...
    while (q--) {
        ...
        switch (op) {
            case 1: {
                i64 x, y;
                cin >> x >> y;
                i64 start = lzt.lower_bound_single_idx(y);
                if (start != -1 && start <= x) {
            ...
            case 2: {
                ...
                i64 ans = 0;
                while (true) {
                    i64 start = lzt.lower_bound_single_idx(y);  // 获取start, log(n)
                    if (start == -1) {
                        break;
                    }
                    start = max(start, x);
                    y += lzt.rangeQuery(1, start - 1);     // log(n)
                    i64 end = lzt.lower_bound_len_idx(y);  // log(n)
                    ans += end - start + 1;
                    x = end + 1;
                    if (x == n + 1) {
                        break;
                    }
                    y -= lzt.rangeQuery(1, end);  // log(n)
                }
                cout << ans << '\n';
            }
        }
