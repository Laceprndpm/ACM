...
signed main(int argc, char **argv)
{
    ...
    BigIntTiny n;
    cin >> n;
    vector<BigIntTiny> arr(n.value());
    vector<BigIntTiny> brr(n.value());
    for (int i = 0; i < n.value(); i++) {
        cin >> arr[i] >> brr[i];
    }
    BigIntTiny M  = accumulate(arr.begin(), arr.end(), BigIntTiny(1LL), multiplies<BigIntTiny>());
    BigIntTiny Mi = 0;
    for (int i = 0; i < n.value(); i++) {
        auto [a, b, r] = exgcd(M / arr[i], arr[i]);
        // (a * M/i) % p = 1
        while (a < 0) {
            a += arr[i];
        }
