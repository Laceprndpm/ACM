...

{
    ...
    r = init();
    p = init();
    vector<int>  sizeSons(n + 1);
    vector<int>  weightChild(n + 1);
    vector<int>  depthNode(n + 1);
    ...
    function<void(int, int)> dfs1 = [&](int root, int last) {
        sizeSons[root] = 1;
        for (int i : graph[root]) {
            if (i == last) continue;
            if (depthNode[i] != 0) continue;
    ...
    int                           dfsCnt = 0;
    vector<int>                   topWeightChainNode(n + 1);
    vector<int>                   DFSnum(n + 1);
    function<void(int, int, int)> dfs2 = [&](int root, int top, int last) {
    ...
    LazySegmentTree<Info, Tag>    lazyST_obj(brr);
    function<void(int, int, int)> update = [&](int x, int y, int z) {
    ...
    function<i64(int, int)> query = [&](int x, int y) {
        i64 ans = 0;
        while (topWeightChainNode[x] != topWeightChainNode[y]) {
            if (depthNode[topWeightChainNode[x]] < depthNode[topWeightChainNode[y]]) {
                swap(x, y);
            }
            ans += lazyST_obj.rangeQuery(DFSnum[topWeightChainNode[x]], DFSnum[x]).x;
            x = father[topWeightChainNode[x]];
        }
        ans += lazyST_obj.rangeQuery(min(DFSnum[x], DFSnum[y]), max(DFSnum[x], DFSnum[y])).x;
        return ans;
    };
    for (int i = 0; i < m; i++) {
        int op;
        ...
        switch (op) {
            ...
            case 4:
                x = init();
                print(lazyST_obj.rangeQuery(DFSnum[x], DFSnum[x] + sizeSons[x] - 1).x % p);
                putchar('\n');
                break;
        }
    }
