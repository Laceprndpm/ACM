...
void solve()
{
    int n, m;
    cin >> n >> m;
    vector<Z> dp(1 << ceil_log2(n * m + 2 * m + 1));  // [0, n * m]
    // vector<vector<Z>> board(n, vector<Z>(2 * m + 1));  // [0, 2m] [m]是0
    // for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        i64 len = 1ll << ceil_log2(i * m + 2 * m + 1);
        ntt(dp.data(), len, 1);
        ntt(b, len, 1);
        for (int i = 0; i < len; i++) {
            b[i] *= dp[i];
        ...
        ntt(b, len, -1);
        // convolution(dp.data(), b, n * m + 2 * m + 1);
        Z   curans = 0;
        int jdx;
        for (int i = 0; i < m; i++) {
            b[i] = 0;
        }
        for (jdx = 0; jdx <= (i + 1) * m; jdx++) {
            dp[jdx]    = b[jdx + m];
            b[jdx + m] = 0;
            curans += dp[jdx];
        }
        memset(dp.data() + jdx, 0, (dp.size() - jdx) * sizeof(Z));
        cout << curans << ' ';
    }
    cout << '\n';
}
...
#endif
    freopen("/home/patchouli/Code/ACM/src/hdu/hd10/test.in", "r", stdin);
    freopen("/home/patchouli/Code/ACM/src/hdu/hd10/test.out", "w", stdout);
