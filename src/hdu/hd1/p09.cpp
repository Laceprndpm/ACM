...
void solve()
{
    ...
    for (int i = n; i >= 1; i--) {
        int cur_pos = pos[i];
        mxl = min(mxl, cur_pos), mxr = max(mxr, cur_pos);
        fk.add(cur_pos, 1);
        int max_val = 0, min_val = 0;
        if (mxr > cur_pos) max_val = mxr - cur_pos + 1 - fk.rangeSum(cur_pos + 1, mxr);
        if (mxl < cur_pos) min_val = cur_pos - mxl + 1 - fk.rangeSum(mxl + 1, cur_pos);
