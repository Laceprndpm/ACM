...
Z work(i64 num1, i64 num0)
{
    Z res;
    for (i64 i = 1; i <= min(num1, num0); i++) {
        res += comb.binom(num1, i) * comb.binom(num0, i);
    }
    return res;
}
...
signed main(int argc, char **argv)
{
    ...
    // 7 2
    // 1100110
    // (C(2, 1) * C(2, 1) + C(2, 2) * C(2, 2)) * 2 + C(2, 1) * C(3, 1) + C(2, 2) * C(3, 2) == (4 + 1) * 2 + 6 + 3
    // 先枚举i太难，在k个数里选i个，还得判断最远距离
    // 如何去重？
    int n, k;
    cin >> n >> k;
    string s;
    cin >> s;
    vector<int> prefix0(n + 1);
    for (int i = 0; i < n; i++) {
    ...
    Z ans = 1;
    while (j < n) {
        ans += work(k, prefix0[j + 1] - prefix0[i]);
        while (s[i] == '0') {
            i++;
        }
        i++;
        if (j != n - 1) ans -= work(k - 1, prefix0[j + 1] - prefix0[i]);
        j++;
        while (j + 1 < n && s[j + 1] == '0') {
            j++;
        }
