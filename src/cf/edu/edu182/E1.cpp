...
struct LazySegmentTree {
    ...
    void modify2(int p, int l, int r, int x, const Z v)
    {
        if (r - l == 1) {
            info[p].x += v;
            return;
    ...
    void modify2(int p, const Z &v) { modify2(1, 0, n, p, v); }
...
void solve()
{
    ...
    for (int i = n; i >= 1; i--) {
        if (ai[i] > mx) {
            suffix.push_back(ai[i]);
            mx = ai[i];
        }
    }
    int         prelen = prefix.size() - 1;
    int         suflen = suffix.size() - 1;
    vector<int> preid(prelen), sufid(suflen);
    iota(all(preid), 0), iota(all(sufid), 0);
    LazySegmentTree<Info, Tag> dp1(prelen + 1);
    LazySegmentTree<Info, Tag> dp2(suflen + 1);

    dp1.modify(0, {1});
    dp2.modify(n + 1, {1});

    for (int i = 1; i <= n; i++) {
        auto         it  = lower_bound(all(prefix), ai[i]);
        pair<int, Z> add = {-1, 0};
        if (it != prefix.end() && *it == ai[i]) {
            add.first  = it - prefix.begin();
            add.second = dp1.rangeQuery(it - prefix.begin() - 1, it - prefix.begin()).x;
        }
        auto itleft = lower_bound(all(prefix), ai[i]);
        dp1.rangeApply(itleft - prefix.begin(), prelen + 1, {2});

        if (add.fi != -1) {
            dp1.modify2(add.fi, {add.second});
        }
    }

    for (int i = n; i >= 1; i--) {
        auto         it  = lower_bound(all(suffix), ai[i]);
        pair<int, Z> add = {-1, 0};
        if (it != suffix.end() && *it == ai[i]) {
            add.first  = it - suffix.begin();
            add.second = dp2.rangeQuery(it - suffix.begin() - 1, it - suffix.begin()).x;
        }

        auto itright = lower_bound(all(suffix), ai[i]);
        dp2.rangeApply(itright - suffix.begin(), suflen + 1, {2});  // 对 >= ai[i] 的区间加

        if (add.first != -1) {
            dp2.modify2(add.first, {add.second});
        }
    }
    Z           ans = 0;
    vector<int> idx;
    ...
    vector<Z> prefixdp(idx.size() + 1);
    for (int i = 0; i < idx.size(); i++) {
        // ans += dp1[idx[i] - 1][prelen - 1] * dp2[idx[j] + 1][suflen - 1] * power(Z(2), (max(idx[j] - idx[i] - 1,
        // 0)));
        prefixdp[i + 1] = prefixdp[i] + dp1.rangeQuery(idx[i] - 1, idx[i]).x;
    }
    Z suffixval = 0;
    for (int i = idx.size() - 1; i >= 0; i--) {
        suffixval += dp2.rangeQuery(idx[i] + 1, idx[i] + 2).x;
        ans += suffixval * prefixdp[i + 1];
    }
    cout << ans << '\n';
}
