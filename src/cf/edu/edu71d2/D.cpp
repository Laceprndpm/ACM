...
void solve()
{
    ...
    sor(arrA), sor(arrB), sor(arr);
    Z    ansA = 1, ansB = 1, anst = 1;
    auto work1 = [&](auto &arr, Z &ans) {
        for (int i = 0; i < n;) {
            int j = i;
    ...
    work1(arrA, ansA), work1(arrB, ansB);
    work1(arr, anst);
    Z bias = 1;
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < 2; j++) {
    ...
    Z ans = comb.fac(n) - ansA - ansB + anst * bias;
    cout << ans << '\n';
    // all - a - b + ab
    // a = b = [0 0 0 0] [1 1 1 1], 0 cnt! * 1 cnt!...
    // both = [{0, 1}, {0, 1}]
    //
}
