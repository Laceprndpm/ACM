...
void solve()
{
    ...
    array<My_Bitset, 2> dp({k + 1, k + 1});  // 到第i层，花了j个白是否可行
    ...
    for (int i = 0; i < shallow; i++) {
        ...
        if (level_size[i + 1] <= k + 1)
            dp[1].or_to_range(level_size[i + 1], k + 1, dp[0].slice(0, k + 1 - level_size[i + 1]));
        dp[1].or_to_range(0, k + 1, dp[0]);
        dp[1].reset_range(atleastwhite, nextatleast);
        if (dp[1].any()) mx = i + 1;
        std::swap(dp[0], dp[1]);
        dp[1].reset();
    }
