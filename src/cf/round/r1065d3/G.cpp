...
struct Comb {
    ...
    {
        if (n < m || m < 0) return 0;
        if (n >= MOD || m >= MOD) return binom(n / MOD, m / MOD) * binom(n % MOD, m % MOD);
...
void solve()
{
    ...
    Z           ans = 1;
    vector<i64> cnt(multime + 1);
    {
        vector<i64> vararr = arr;
        for (int i = 1; i <= n; i++) {
            vararr[i] *= (1ll << multime);
        }
        for (int p = multime; p >= 0; p--) {
            for (int i = 1; i <= n; i++) {
                int cur = 0;
                // while (vararr[i] + (1 << p) <= brr[i]) {
                //     cur++;
                //     vararr[i] += (1 << p);
                // }
                cur = (brr[i] - vararr[i]) / (1ll << p);
                vararr[i] += cur * (1ll << p);
                cnt[p] += cur;
                ans = ans * comb.binom(cnt[p], cur);
            }
        }
    }
    i64 ct = accumulate(all(cnt), 0ll) + multime;
    cout << ct << ' ' << ans << '\n';
}
