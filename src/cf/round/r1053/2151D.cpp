...
void solve()
{
    ...
    for (int i = (n + 1) / 2; i >= 1; i--) {
        ans *= comb.binom(ci, arr[i]);
        ci -= arr[i];
        ci += 2;
    }
    cout << ans << '\n';
    // deque<int> breakpoint{0};
    ...
    //     }
    // }
    // while (breakpoint.size() > 1) {
    //     int last = breakpoint.front();
    //     breakpoint.pop_front();
    //     int         cur   = breakpoint.front();
    //     vector<int> cot   = {arr[last + 1] - 2};
    //     int         total = arr[last + 1] - 2;
    //     for (int i = last + 2; i <= cur; i++) {
    //         cot.pb(arr[i]);
    //         total += arr[i];
    //     }
    //     Z curans = comb.fac(total);
    //     for (int i : cot) {
    //         curans /= comb.fac(i);
    //     }
    //     ans *= curans;
