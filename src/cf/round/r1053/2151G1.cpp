...
void solve()
{
    ...
    auto dfs = [&](this auto&& self, int l, int r, vector<int> has1, vector<int> has2) -> int {
        if (has2.size() == 1) {
            return has2[0];
        }
        int         mid = (l + r) / 2;
        ...
        if (lft1.size() + lft2.size() * 2 != (mid - l + 1)) {
            return self(l, mid, lft1, lft2);
        } else {
            return self(mid + 1, r, rgt1, rgt2);
        }
    };
    vector<int> al(n);
    iota(all(al), 1);
    int res = dfs(1, 2 * n - 1, vector<int>(), al);
    cout << "! " << res << endl;
}
