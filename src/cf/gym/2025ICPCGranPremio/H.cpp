...
void solve()
{
    int n, m;
    cin >> n >> m;
    vector<vector<Z>> dp_front(n + 1, vector<Z>(m + 2));
    vector<vector<Z>> dp_back(n + 2, vector<Z>(m + 3));
    string            s, t;
    cin >> s >> t;
    s                 = ' ' + s;
    t                 = ' ' + t;
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= m; j++) {
            dp_front[i][j] = dp_front[i - 1][j];
            if (s[i] != t[j]) continue;
            dp_front[i][j] += dp_front[i - 1][j - 1];
        }
    }
    for (int i = n; i >= 1; i--) {
        for (int j = 1; j <= m + 1; j++) {
            dp_back[i][j] = dp_back[i + 1][j];
            if (s[i] != t[j]) continue;
            dp_back[i][j] += dp_back[i + 1][j + 1];
        }
    }
    Z ans = 0;
    for (int i = 1; i <= n; i++) {
        for (char c = 'a'; c <= 'z'; c++) {
            if (c == s[i]) continue;
            for (int j = 0; j <= m; j++) {  // c不涉及
                ans += dp_front[i - 1][j] * dp_back[i + 1][j + 1];
            }
            for (int j = 1; j <= m; j++) {
                if (c != t[j]) continue;
                ans += dp_front[i - 1][j - 1] * dp_back[i + 1][j + 1];
            }
        }
    }
    ans += dp_front[n][m];
