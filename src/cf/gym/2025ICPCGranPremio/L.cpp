...
struct LazySegmentTree {
    ...
    Info rangeApply1(int p, int l, int r, int x, int y, const Tag &v)
    {
        if (l >= y || r <= x) {
            return Info{};
        }
        ...
        int m = (l + r) / 2;
        push(p);
        auto resl = rangeApply1(2 * p, l, m, x, y, v);
        auto resr = rangeApply1(2 * p + 1, m, r, x, y, v);
        pull(p);
        return resl + resr;
    }

    Info rangeApply1(int l, int r, const Tag &v) { return rangeApply1(1, 0, n, l, r, v); }
...
struct HLD {
    ...
    Z query(int u, int v, int x)
    {
        Z ans = 0;
        while (top[u] != top[v]) {
            if (dep[top[u]] > dep[top[v]]) {
                ans += seg.rangeApply1(in[top[u]], in[u] + 1, Tag{x}).x;
                u = parent[top[u]];
            } else {
                ans += seg.rangeApply1(in[top[v]], in[v] + 1, Tag{x}).x;
                v = parent[top[v]];
            }
        }
        if (in[u] > in[v]) {
        ...
        ans += seg.rangeApply1(in[u] + 1, in[v] + 1, {x}).x;
        return ans;
    }
