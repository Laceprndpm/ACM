...
void solve()
{
    ...
    auto dfs2 = [&](this auto &&self, int u, int fa) -> void {
        Z         cur    = 1;
        int       sonssz = sons[u].size();
        vector<Z> prefix(sonssz, 1);
        vector<Z> suffix(sonssz, 1);
        for (int i = 0; i <= sonssz - 2; i++) {
            prefix[i + 1] = prefix[i] * (dp[sons[u][i]][0] + 1);
        }
        for (int i = sonssz - 1; i >= 1; i--) {
            suffix[i - 1] = suffix[i] * (dp[sons[u][i]][0] + 1);
        }
        for (int i = 0; i < sonssz; i++) {
            int v    = sons[u][i];
            dp[v][1] = prefix[i] * suffix[i] * (dp[u][1] + 1);
        }
        for (int v : adj[u]) {
            if (v == fa) continue;
            self(v, u);
    ...
    dfs2(1, 0);
    vector<array<Z, 2>> ndp(n + 1);
    for (int i = 1; i <= n; i++) {
        ndp[i][0] = dp[i][0] * (dp[i][1] + 1);  // 该点被哪些子图包含
        ndp[i][1] = dp[i][0] * dp[i][1];        // u - fa被哪些子图包含
    }
    vector<vector<int>>         stParent(17, vector<int>(n + 1));
    vector<vector<array<Z, 2>>> stdp(17, vector<array<Z, 2>>(n + 1));
    for (int i = 1; i <= n; i++) {
        stParent[0][i] = parent[i];
        stdp[0][i][0]  = ndp[i][0];
        stdp[0][i][1]  = ndp[i][1];
    }
    for (int i = 1; i < 17; i++) {
        for (int j = 1; j <= n; j++) {
            stParent[i][j] = stParent[i - 1][stParent[i - 1][j]];
            stdp[i][j][0]  = stdp[i - 1][j][0] + stdp[i - 1][stParent[i - 1][j]][0];
            stdp[i][j][1]  = stdp[i - 1][j][1] + stdp[i - 1][stParent[i - 1][j]][1];
        }
    }
    auto query = [&](int u, int v) -> Z {
        Z nodes  = 0;
        Z chains = 0;
        if (dep[u] < dep[v]) {
            swap(u, v);
        ...
        for (int i = 16; i >= 0; i--) {
            if (stParent[i][u] != stParent[i][v]) {
                nodes += stdp[i][u][0];
                chains += stdp[i][u][1];
                nodes += stdp[i][v][0];
                chains += stdp[i][v][1];
                u = stParent[i][u], v = stParent[i][v];
            }
        }
        assert(u != v);
        nodes += ndp[u][0];
