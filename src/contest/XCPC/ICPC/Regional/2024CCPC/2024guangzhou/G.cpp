...
struct Info {
    ...
    void apply(const Tag &t) & { arr *= t.x; }
};
...
void solve()
{
    ...
    vector<i64> prefix(m + 1, 1);
    prefix[0] = 1;
    for (int i = 0; i < m; i++) {
        prefix[i + 1] = prefix[i] * arr[action[i]];
    }
    ...
    for (int len = 1; len <= m; ++len) {
        for (int l = 0; l + len - 1 < m; ++l) {
            ...
            for (int k = l; k < r; ++k) {
                dp[l][r] = max(dp[l][r], dp[l][k] + dp[k + 1][r] + action[k + 1] - action[k] - 1);
            }
        }
    }
    cout << (dp[0][action.size() - 1] + cnt) % P << '\n';
}
