...
void solve()
{
    ...
    auto dfs  = [&](this auto &&self, int u, int fa) -> void {
        ...
        for (int v : adj[u]) {
            ...
            self(v, u);
            siz[u] += siz[v];
            tmpans2   = tmpans2 + (getsum[u] * getsum[v]);
            getsum[u] = (getsum[u] + getsum[v]);
        }
        getsum[u] = (getsum[u] + (siz[u]) * siz[u]);
        for (int v : adj[u]) {
            if (v == fa) continue;
            ans1 += (2ll * (n - siz[v]) * (n - siz[v])) * ((getsum[v] - (siz[v]) * siz[v]));  // 有序项1
        }
        ans1 += ((n - siz[u]) * (n - siz[u])) * ((siz[u]) * siz[u]);  // 平方项
        ans1 = (ans1 + 2ll * tmpans2);
    };
    dfs(1, 0);
    ans1 = ans1 / Z((n - 1) * n / 2);
    ans1 = ans1 / Z((n - 1) * n / 2);
