...
void solve()
{
    // 存关系，如果存在a_i > a_(i+1)记为1
    // 如果直接ln套log能过吗？
    // (n / 1 + n / 2 + ... n / n) * ln(n) = 3e7，常数小应该能过？
    // 牛魔，漏了个n
    //
    // 如果只更新涉及的呢？每次会有两个数被更新
    // d(x) = sqrt(x)
    // sqrt(2e5) * log(2e5) * 2e5 = 1e9，很可能被卡
    //
    //
    //
    int n, q;
    ...
    vector<int> antiinver(n + 1);
    for (int i = 1; i < n; i++) {
        if (arr[i] > arr[i + 1]) {
            invernum++;
            for (int sons : factors[i]) {
                antiinver[sons]++;
            }
        }
    }
    ...
    auto update = [&](int id, int cmd) -> void {
        cnt[antiinver[id]]--;
        antiinver[id] += cmd;
        cnt[antiinver[id]]++;
        return;
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (p != 1 && arr[p - 1] > arr[p]) {
            invernum--;
            for (int sons : factors[p - 1]) {
                update(sons, -1);
            }
        }
        if (p != n && arr[p] > arr[p + 1]) {
            invernum--;
            for (int sons : factors[p]) {
                update(sons, -1);
            }
        }
        arr[p] = v;
