...
void solve()
{
    ...
    vector<vector<int>> ci(m + 1, vector<int>(n + 1));
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            cin >> ci[i][j];
        }
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            mnf.addEdge(i, j + m, INF, -ci[i][j]);
            mxf.addEdge(i, j + m, INF, ci[i][j]);
        }
    }
    for (int i = 1; i <= m; i++) {
        mxf.addEdge(0, i, has[i], 0);
        mnf.addEdge(0, i, has[i], 0);
    }
    for (int j = 1; j <= n; j++) {
        mxf.addEdge(j + m, n + m + 1, need[j], 0);
        mnf.addEdge(j + m, n + m + 1, need[j], 0);
    }
    auto [_, res1]  = mxf.flow(0, n + m + 1);
    auto [__, res2] = mnf.flow(0, n + m + 1);
