...
struct MCFGraph {
    struct Edge {
        ...
        Edge(int v, int c, int f) : to(v), cap(c), f(f) {}
    };
    ...
    std::pair<int, i64> flow(int s, int t)
    {
        ...
        do {
            ...
            for (int i = t; i != s; i = e[pre[i] ^ 1].to) aug = std::min(aug, e[pre[i]].cap);
...
void solve()
{
    ...
    int p, q;
    cin >> p >> q;
    MCFGraph mcf((p + 1) * (q + 1) + 2);
    auto     zip = [&](int i, int j) { return i * (q + 1) + j; };
    for (int i = 0; i <= p; i++) {
        for (int j = 0; j < q; j++) {
            int val;
            cin >> val;
            mcf.addEdge(zip(i, j), zip(i, j + 1), 1, -val);
            mcf.addEdge(zip(i, j), zip(i, j + 1), INF, 0);
        }
    }
    for (int j = 0; j <= q; j++) {
        for (int i = 0; i < p; i++) {
            int val;
            cin >> val;
            mcf.addEdge(zip(i, j), zip(i + 1, j), 1, -val);
            mcf.addEdge(zip(i, j), zip(i + 1, j), INF, 0);
        }
    }
    int s = (p + 1) * (q + 1);
    int t = s + 1;
    for (int i = 0; i < a; i++) {
        int k, x, y;
        cin >> k >> x >> y;
        mcf.addEdge(s, zip(x, y), k, 0);
    }
    for (int i = 0; i < b; i++) {
        int r, x, y;
        cin >> r >> x >> y;
        mcf.addEdge(zip(x, y), t, r, 0);
    }
    auto [_, f] = mcf.flow(s, t);
    cout << -f << '\n';
}
