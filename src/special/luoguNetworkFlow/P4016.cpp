...
void solve()
{
    ...
    i64      sum = accumulate(all(arr), 0);
    MCFGraph mcf(n + 2);
    i64      arrange = sum / n;
    for (int i = 1; i <= n; i++) {
        if (arr[i] > arrange) {
            mcf.addEdge(0, i, arr[i] - arrange, 0);
            for (int j = 1; j <= n; j++) {
                if (arr[j] >= arrange) continue;
                mcf.addEdge(i, j, INF, min(n - abs(i - j), abs(i - j)));
            }
        }
    }
    for (int i = 1; i <= n; i++) {
        if (arr[i] > arrange) continue;
        mcf.addEdge(i, n + 1, arrange - arr[i], 0);
    }
    auto [_, num] = mcf.flow(0, n + 1);
    cout << num << '\n';
}
