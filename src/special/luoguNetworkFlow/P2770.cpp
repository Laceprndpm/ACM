...
struct MCFGraph {
    struct Edge_ {
        ...
        Edge_(int v, int c, int f, int u) : v(v), c(c), f(f), u(u) {}
    };
    ...
    void addEdge(int u, int v, int c, int f)
    {
        ...
        e.emplace_back(v, c, f, u);
        g[v].push_back(e.size());
        e.emplace_back(u, 0, -f, v);
    }
...
void solve()
{
    ...
    for (int i = 0; i < v; i++) {
        string su, sv;
        cin >> su >> sv;
        int u = cities[su], v = cities[sv];
        if (u > v) swap(u, v);
        if (u == 1 && v == n) {
            mcf.addEdge(u * 2, v * 2 - 1, 2, 0);
        } else {
            mcf.addEdge(u * 2, v * 2 - 1, 1, 0);
        }
    }
    for (int i = 2; i < n; i++) {
        mcf.addEdge(i * 2 - 1, i * 2, 1, -1);
    }
    mcf.addEdge(1, 2, 2, 0);
    mcf.addEdge(n * 2 - 1, n * 2, 2, 0);
    mcf.addEdge(0, 1, 2, 0);
    mcf.addEdge(n * 2, n * 2 + 1, 2, 0);

    auto [s, res] = mcf.flow(0, n * 2 + 1);
    if (s < 2) {
        cout << "No Solution!\n";
        return;
    }
    ...
    for (auto e : edges) {
        if (e.from % 2 == 0) {
            for (int i = 0; i < e.flow; i++) {
                int u = e.from / 2, v = (e.to + 1) / 2;
                if (u < 1 || u > n || v < 1 || v > n) continue;
                graph[e.from / 2].pb((e.to + 1) / 2);
            }
        }
    }
    vector<vector<int>> path;
    auto                dfs = [&](auto&& dfs, int u, vector<int> rema) -> void {
        rema.pb(u);
        for (int v : graph[u]) {
            dfs(dfs, v, rema);
        }
    ...
    realpath.erase(unique(all(realpath)), realpath.end());
    for (int i : realpath) {
        cout << names[i] << '\n';
    }
}
