...
int solve()
{
    ...
    if (n == 0) return 0;
    cin >> m >> x1 >> y1 >> x2 >> y2;
    vector<pair<Point, Point>> vec1(n);
    for (int i = 0; i < n; i++) {
        int ui, li;
        cin >> ui >> li;
        Point a = {T(ui), T(y1)}, b = {T(li), T(y2)};
        vec1[i] = {a, b};
    }
    vec1.pb({Point{T(x2), T(y1)}, Point{T(x2), T(y2)}});
    auto        onleft = [&](Point x, pair<Point, Point> line) { return ((x - line.fi) ^ (line.se - line.fi)) > 0; };
    vector<int> cnt(n + 1);
    for (int i = 0; i < m; i++) {
        int x, y;
        cin >> x >> y;
