...
void solve()
{
    ...
    while (b1 < n) {
        ...
        // 123
        // b1 = 2 b2 = 3 b3 = 4
        if (it == ord[val].begin()) {  // 第一次出现，b2从(第二个,最后一个]之间都可以
            dbg(*next(it) + 1);
            dbg(ord[val].bk + 1);
            seg.rangeApply(*next(it) + 1, ord[val].bk + 1, Tag{1});
        } else if (next(it) != ord[val].end()) {  // 第二次出现，则只要缩小范围
            dbg(*it + 1);
            dbg(*next(it) + 1);
            seg.rangeApply(*it + 1, *next(it) + 1, Tag{-1});
        }
        b1++;
        auto res   = seg.rangeQuery(b1 + 1, n + 1);
        int  cur   = res.x;
        int  curb2 = res.id;
        if (cur > ans) {
            b1b2 = {b1, curb2};
