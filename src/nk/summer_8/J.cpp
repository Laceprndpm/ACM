...
void solve()
{
    ...
    vector<Z> a_vec(len), b_vec(len);
    for (int i = 0; i < a.size(); i++) {
        a_vec[i] = a[a.size() - i - 1] - '0';
    }
    for (int i = 0; i < b.size(); i++) {
        b_vec[i] = b[b.size() - i - 1] - '0';
    }
    ntt(a_vec.data(), len, 1);
    ntt(b_vec.data(), len, 1);
    dbg(a_vec);
    dbg(b_vec);
    for (int i = 0; i < len; i++) {
        a_vec[i] *= b_vec[i];
