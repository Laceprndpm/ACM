...
inline void pre_dfs(int x, int par = 0, int deep = 1)
{
    ...
    fa[x]  = par;
    for (int y : g[x])
        if (y != par) pre_dfs(y, x, deep + 1), sz[x] += sz[y];
}

inline void go(int x, int tpf = root, int par = 0)
{
    bg[x]     = ++ccnt;
    ord[ccnt] = x, top[x] = tpf;
    int mx = -1, pos = 0;
    for (int y : g[x])
        if (y != par) {
            if (sz[y] > mx) {
...
struct segt {
    ...
    int find(int x, int l, int r)
    {
        ...
        int md = l + r >> 1;
        if (val[x << 1 | 1])
            return find(x << 1 | 1, md + 1, r);
        else
            return find(x << 1, l, md);
    }
    ...
    for (; q--;) {
        ...
        if (op == 1) {
            ...
            while (dep[l] >= dep[zx]) {  // include zx
                int lt = lst(l);
                if (dep[lt] >= dep[zx]) {
                    for (int i = 0; i < min(22, dep[l] - dep[lt]); ++i)
                        vl[sl++] = 1;
                    vl[sl++] = a[lt];
                    l        = lt;
                    l        = fa[l];
                    ...
                    for (int i = 0; i < min(22, dep[l] - dep[zx]); ++i)
                        vl[sl++] = 1;
                    vl[sl++] = a[zx];
                    break;
                }
            ...
            while (dep[r] > dep[zx]) {
                int lt = lst(r);
                if (dep[lt] > dep[zx]) {
                    for (int i = 0; i < min(22, dep[r] - dep[lt]); ++i)
                        vr[sr++] = 1;
                    vr[sr++] = a[lt];
                    r        = lt;
                    r        = fa[r];
                    ...
                    for (int i = 0; i < min(22, dep[r] - dep[zx]); ++i)
                        vr[sr++] = 1;
                    break;
                }
                if (cnt > 12) {
