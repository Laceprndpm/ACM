...
// BEGIN: nt/coprime_factorization.hpp
...
pair<vc<T>, vvc<pair<int, int>>> coprime_factorization(vc<T> nums)
{
    ...
    return {basis, res};
}  // END: nt/coprime_factorization.hpp
...
mint fact(int n)
{
    ...
    while (len(dat) <= n)
        dat.eb(dat[len(dat) - 1] * mint(len(dat)));
    return dat[n];
}
...
void solve()
{
    ...
    vi A          = {a, c};
    vi E          = {b, d};
    auto [X, pfs] = coprime_factorization<ll>(A);
    using mint    = modint998;

    vc<i128> F(len(X), infty<i128>);
    FOR(k, 2)
    {
        map<ll, i128> MP;
        for (auto &[a, b] : pfs[k]) {
